...
Lagoon MVP is a local-first, single-user lecture assistant. It helps one user create a local lecture workspace, import one lecture, and later produce grounded transcript-backed notes, summaries, and tutor answers for the selected lecture.

## Context

Phase order starts with local app foundation, then media import, transcription, provenance, understanding, simple local retrieval, transcript-grounded summaries, tutor practice, post-MVP improvement proposals, hardening, and deferred cloud sync decision. `SP-00` must remove product ambiguity before implementation begins.
...
- Full tutor/research suite first: rejected because media import, transcript, provenance, and local memory must exist before reliable tutoring.
